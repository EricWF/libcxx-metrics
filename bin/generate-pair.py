...
Today you are writing calls to member functions of std::pair.
...
* The parameters T1, T2, U1, U2, P, Args1..., and Args2... are already defined.
...
template <class T1, class T2, class U1, class U2, class P, class ...Args1, class ...Args2>
struct Tester<T1, T2, U1, U2, P, Pack<Args1...>, Pack<Args2...> > {
 using first_type = T1;
...
c.user("Improve the following prompt. It's original purpose was writing C++ tests for std::algorithm. The std::pair. Please rewrite it to prompt the assistant to write tests for std::unordered_map")
...
template <class T1, class T2, class U1, class U2, class P, class Pack1, class Pack2>
struct Tester;
...
template struct Tester<int, void*, long, int*, PairLike<int, void*>, Pack<long>, Pack<long*> >;
'''
...
if not JSON_P.is_file():
  jl = JobList()
  lines = json.loads(Path('/tmp/out.json').read_text())
  for l in lines:
